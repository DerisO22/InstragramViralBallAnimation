...
addEventListener("keydown", (event) => {
  ...
  switch (event.key.toLowerCase()) {
    ...
    case "r": // Reset/center camera
      ...
      break;
    case "arrowup": // Manual camera movement
    case "w":
      event.preventDefault();
      if (camera.followingEnabled === false) {
      ...
      break;
    case "arrowdown": // Manual camera movement
    case "s":
      event.preventDefault();
      if (camera.followingEnabled === false) {
        moveCameraManually(0, 50);
      ...
      break;
    case "arrowleft": // Manual camera movement
    case "a":
      event.preventDefault();
      if (camera.followingEnabled === false) {
      ...
      break;
    case "arrowright": // Manual camera movement
    case "d":
      event.preventDefault();
      if (camera.followingEnabled === false) {
        moveCameraManually(50, 0);
...
setInterval(() => {
  if (gameStarted && performance.memory) {
    ...
    if (memUsage > 100 && ballObjects.length > 100) {
      console.log(
        `⚠️ High memory usage: ${memUsage.toFixed(
          2
...
- Can handle up to ${MAX_BALLS} balls
- Auto-adjusts quality based on count
- Memory usage monitoring
- Aggressive cleanup when needed
- Zero memory leaks guaranteed

🎯 STRESS TEST:
Press '0' to spawn 500 balls instantly!
