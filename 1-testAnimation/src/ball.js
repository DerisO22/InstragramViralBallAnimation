...
const MAX_BALLS = 10000; // Much higher limit now that we're memory efficient
const MAX_PARTICLES = 50; // Reduced for memory efficiency
const AUDIO_POOL_SIZE = 15; // Increased for better priority audio
const TRAIL_UPDATE_INTERVAL = 3; // Update trails less frequently
...
function playNote(frequency, duration = 0.5, velocity = 0.7, priority = false) {
  ...
  const allowAudio =
    ...
    ballObjects.length <= 10; // Always play for first 10 balls
  ...
  for (let i = 0; i < maxHarmonics; i++) {
    ...
    const threshold = priority ? 0.05 : 0.1; // Lower threshold for priority sounds
